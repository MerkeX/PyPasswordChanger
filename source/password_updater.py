...
#
# If you have already created a previous file with this tool before
# you can get username and sitename list from the previous file
# without having to collect them from a browser file again
#
...
def execute():
    ...
    if(pass_text_num == 'y' or pass_text_num == 'Y'):
        if (os.path.isfile(path + "list_clear_and_encrypted_passwords_new.csv'") == 'True'):
            os.remove(path + "list_clear_and_encrypted_passwords_new.csv'");
        num_files = 1;
    ...
    if(num_files == 1):
        with open(path + 'list_clear_and_encrypted_passwords_new.csv',mode = 'w',newline = "\n") as list_clear_encrypted_passwords:
